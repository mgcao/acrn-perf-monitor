...
bool acrn_vmexit_init(void)
{
	...
	printf("ACRN VMEXIT base, GPA=0x%x, map to: 0x%p", ACRN_VMEXIT_GPA_BASE, vmexit_vaddr);
	vmexit_magic_flag = vmexit_vaddr;

	if (*vmexit_magic_flag == ACRN_MAGIC_FLAG) {
		printf("ACRN-VMEXIT magic flag: 0x%lx matched!", ACRN_MAGIC_FLAG);
	} else {
		printf("ACRN-VMEXIT not supported! magic flag not matched, read: 0x%lx, shall be 0x%lx!", *vmexit_magic_flag, ACRN_MAGIC_FLAG);
		return false;
	}

	vmexit_cnt = vmexit_vaddr + 1;
	vmexit_reason = vmexit_vaddr + 2;
...
bool acrn_vmexit_end(void)
{
	...
	if (vmexit_cnt_stop > vmexit_cnt_start) {
		...
		vmexit_info[2] = *vmexit_tsc_start;
		vmexit_info[3] = *vmexit_tsc_stop;
...
{
	printf("vmexit: cnt delta:%lu (%lu - %lu), last one: 0x%02lx  tsc delta: %lu\n",
			vmexit_info[0],  vmexit_cnt_stop, vmexit_cnt_start,
			vmexit_info[1], vmexit_info[3] - vmexit_info[2]);
}